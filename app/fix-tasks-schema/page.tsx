...
import { CheckCircle, AlertCircle, Play, Loader2, Database } from 'lucide-react'
...
export default function FixTasksSchemaPage() {
  ...
  const checkSchemaStatus = async () => {
    ...
    try {
      const response = await fetch('/api/fix-tasks-schema')
  ...
  const requiredColumns = ['priority', 'build_stage', 'estimated_hours']
      ...
      <div className="space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <h1 className="text-3xl font-bold">Fix Tasks Schema</h1>
          </div>
          <p className="text-muted-foreground">
            Update the project_tasks table schema to support task priority, build stages, and time estimation.
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Checking the current project_tasks table schema...
            </CardDescription>
          </CardHeader>
          <CardContent>
            {checkingStatus ? (
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium">Adds priority column</p>
                  <p className="text-sm text-muted-foreground">
                    Allows tasks to have low, medium, or high priority levels
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2" />
                <div>
                  <p className="font-medium">Adds build_stage column</p>
                  <p className="text-sm text-muted-foreground">
                    Links tasks to specific build stages (planning, disassembly, painting, etc.)
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2" />
                <div>
                  <p className="font-medium">Adds estimated_hours column</p>
                  <p className="text-sm text-muted-foreground">
                    Enables time estimation and project planning features
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2" />
                <div>
                  <p className="font-medium">Adds database constraints</p>
                  <p className="text-sm text-muted-foreground">
                    Ensures data integrity with proper validation rules
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2" />
                <div>
                  <p className="font-medium">Creates performance indexes</p>
                  <p className="text-sm text-muted-foreground">
                    Improves query performance for task filtering and sorting
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  {result.details && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        <h5 className="font-medium mb-2">Constraints Added:</h5>
                        <ul className="space-y-1">
